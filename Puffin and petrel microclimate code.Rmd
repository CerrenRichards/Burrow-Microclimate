...
ATPU_LSSP %>% group_by(Species, Burrow) %>% 
  mutate(Logger = ifelse(Inside > Outside, 1, 0), ## Assign yes if the burrow temperatures are greater than the outside loggers
         Station = ifelse(Inside > Temperature, 1,0)) %>% ## Assign yes if the burrow temperatures are greater than the weather station
