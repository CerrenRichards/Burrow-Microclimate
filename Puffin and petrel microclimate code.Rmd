...
## Look at the mean range across burrows
extreme_cold_LSSP %>% group_by(Burrow)%>% 
...
## Look at the mean range across burrows
extreme_cold_ATPU %>% group_by(Burrow)%>% 
  summarise(mean = mean(buffer_kestrel)) %>%
              summarise(min = min(mean),
                        max = max(mean))
...
ATPU_LSSP %>% filter(Species == "Leach's storm-petrel" & Date == "2021-09-11") %>% filter(buffer_kestrel > 0) %>%
              summarise(mean = mean(buffer_kestrel),
                        sd = sd(buffer_kestrel))

## Look at the mean range across burrows
ATPU_LSSP %>% filter(Species == "Leach's storm-petrel" & Date == "2021-09-11") %>% filter(buffer_kestrel > 0) %>% group_by(Burrow)%>%
              summarise(mean = mean(buffer_kestrel)) %>%
              summarise(min = min(mean),
                        max = max(mean))
...
## COLD BUFFERING THROUGH THE WHOLE SEASON
ATPU_LSSP %>% group_by(Species) %>% filter(buffer_kestrel > 0) %>%
              summarise(mean = mean(buffer_kestrel),
                        sd = sd(buffer_kestrel))
...
## Summarise the temperature variation during the extremes
# LOGGERS
mean(extreme_cold_ATPU$Outside, na.rm=T) # 9.4
min(extreme_cold_ATPU$Outside, na.rm=T) # 6.3
sd(extreme_cold_ATPU$Outside, na.rm=T) # 1.6

# Weather station
extreme_cold_ATPU %>% filter(Burrow == 42) %>%
  summarise(mean = mean(Temperature), #9.1
            min = min(Temperature), # 8.2
            sd = sd(Temperature)) #0.7

# LOGGERS
mean(extreme_cold_LSSP$Outside, na.rm=T) # 6.5
min(extreme_cold_LSSP$Outside, na.rm=T) # 4.5
sd(extreme_cold_LSSP$Outside, na.rm=T) # 0.9

# Station
extreme_cold_LSSP %>% filter(Burrow == 21) %>%
  summarise(mean = mean(Temperature), # 6
            min = min(Temperature),# 5.3
            sd = sd(Temperature)) #0.3

## Hurricane
ATPU_LSSP %>% filter(Species == "Leach's storm-petrel" & Date == "2021-09-11") %>%
              summarise(mean = mean(Temperature), # 18.2
                        min = min(Temperature),# 14.9
                        sd = sd(Temperature), #1.3
                        mean.out = mean(Outside), # 17.7
                        min.out = min(Outside),#13.1
                        sd.out = sd(Outside)) #1.7
...
puffin.TNZ<-ggplot(data = ATPU.TNZ, aes(x = Burrow2, y = Inside))+
    annotate("rect", xmin=-Inf, xmax=Inf, ymin=13.45, ymax=32, alpha=0.2)+
  geom_violin()+
...
petrel.TNZ<-ggplot(data = LESP.TNZ, aes(x = Burrow2, y = Inside))+
    annotate("rect", xmin=-Inf, xmax=Inf, ymin=24, ymax=32, alpha=0.2)+
  geom_violin()+
 stat_summary(fun="mean", geom="point", color = "black", size = 1)+ 
  xlab("Burrow") + ylab("Temperature (°C)")+
  plottheme+
...
## Proportion of time outside LCT per burrow
prop_refuge.atpu <- ATPU_LSSP %>% filter(Species == "Atlantic puffin") %>% 
  mutate(Station = ifelse(Inside < 13.44, 1,0)) %>% ## Assign 1 if the burrow temperatures are greater than the weather station
  select(Burrow,  Station) %>%
  gather(Temp, Refuge, Station) %>% # gather data
  group_by(Burrow, Temp) %>%
  summarise(n=n(),
            Yes = sum(Refuge, na.rm = T),
            prop = (Yes/n)*100)

mean(prop_refuge.atpu$prop)
sd(prop_refuge.atpu$prop)
...
## Proportion of time outside LCT per burrow
prop.extreme_cold_ATPU <- extreme_cold_ATPU %>% filter(Species == "Atlantic puffin") %>% 
  mutate(Station = ifelse(Inside < 13.44, 1,0)) %>% ## Assign 1 if the burrow temperatures are greater than the weather station
  select(Burrow,  Station) %>%
  gather(Temp, Refuge, Station) %>% # gather data
  group_by(Burrow, Temp) %>%
...
mean(prop.extreme_cold_ATPU$prop)
sd(prop.extreme_cold_ATPU$prop)
...
prop_refuge %>% filter(Temp == "Station") %>% group_by(Species) %>%
...
## Season and hour composite plot
...
# Create the plot
dayplot<- ggplot(data = maxmin_season, aes(x = Time, y = Mean, group = Location))+
  geom_ribbon(aes(ymin=Mean-sd, ymax=Mean+sd, fill = Location), alpha = 0.35)+
  geom_line(aes(colour = Location),  size = 1.25)+
  xlab("Time of Day (h)") + ylab("Temperature (°C)")+
  scale_x_continuous(breaks=seq(0,23,4))+
...
tempall <- ggplot(data = season, aes(x = datetime, y = mean))+
  ...
  geom_line(aes(x = datetime, y = meankest), 
            colour = "#43A407")+ # Line for weather station
  plottheme+
  xlab("Date")+ theme(axis.title.y = element_blank())+
  facet_wrap(~Species, scales = "free", ncol = 1)+
   theme(strip.text.x = element_blank(), 
          strip.background = element_blank())
...
# Layout the plots
(dayplot |  tempall ) + plot_layout(widths=c(1,2))

# save the plot
ggsave("Composite_day_season.pdf", 
