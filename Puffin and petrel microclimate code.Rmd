...
# For some reason it doesn't allow the two to be pasted under one another
# So here we just make two columns and will join them later
cpt1[i] <- cpts(cpt.mean(x$Inside, method = "BinSeg", Q=2))[1]
cpt2[i] <- cpts(cpt.mean(x$Inside, method = "BinSeg", Q=2))[2]
...
# Identify the change points
cpt1[i] <- cpts(cpt.mean(x$Inside, method = "BinSeg", Q=3))[1]
cpt2[i] <- cpts(cpt.mean(x$Inside, method = "BinSeg", Q=3))[2]
cpt3[i] <- cpts(cpt.mean(x$Inside, method = "BinSeg", Q=3))[3]
...
# Identify the change points
cpt <- cpts(cpt.mean(x$Inside, method = "PELT"))
...
# Heat extreme 2 = 28/30 burrows (93%)
...
x1 <- cpt.meanvar(x$Inside, method = "PELT")
plot(x1)
print(x1)
