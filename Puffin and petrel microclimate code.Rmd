...
## Thermoneutral zone
```{r}
## Create dataframe of TNZ of the species
sp.TNZ <- data.frame(Species = c("Atlantic puffin", "Atlantic puffin","Atlantic puffin",
                               "Leach's storm-petrel","Leach's storm-petrel"), 
                   TNZ = c("UTC.hatchling","LCT.hatchling","LCT.adult",
                           "LCT.hatchling","LCT.adult"), 
                   Temperature = c(35, 32, 13.16,
                                  32, 24))

LCT <- data.frame(Species = c("Atlantic puffin","Leach's storm-petrel"), 
                   LCT =  c(13.16,24))
...
puffin.TNZ<-ggplot(data = ATPU.TNZ, aes(x = Burrow2, y = Inside))+
    annotate("rect", xmin=-Inf, xmax=Inf, ymin=13.16, ymax=35, alpha=0.2)+
  geom_violin()+
 stat_summary( fun.data = "mean_sdl",  fun.args = list(mult = 1), 
    geom = "pointrange", color = "black", size = 0.1)+
  xlab("Burrow") + ylab("Temperature (°C)")+
  plottheme+
  geom_hline(yintercept = 13.15, colour = "#414487FF", linetype = "dashed", size = 1)+
  geom_hline(yintercept = 32, colour = "#89B4C7", linetype = "dashed", size = 1)+
  geom_hline(yintercept = 35, colour = "#FCA636FF", linetype = "dashed", size = 1)+
...
petrel.TNZ<-ggplot(data = LESP.TNZ, aes(x = Burrow2, y = Inside))+
 ...
 stat_summary( fun.data = "mean_sdl",  fun.args = list(mult = 1), 
  ...
  xlab("Burrow") + ylab("Temperature (°C)")+
  plottheme+
  geom_hline(yintercept = 24, colour = "#414487FF", linetype = "dashed", size = 1)+
  geom_hline(yintercept = 32, colour = "#89B4C7", linetype = "dashed", size = 1)+
  facet_wrap(~Species)+
    theme(strip.text.x = element_text(size = 16, color = "white", face = "bold"),
        strip.text.y = element_text(size = 16, color = "white", face = "bold"),
...
## Puffin plot
ggplot(data = TNZ, aes(x = Burrow2, y = Inside))+
    annotate("rect", xmin=-Inf, xmax=Inf, ymin=13, ymax=35, alpha=0.2)+
  geom_violin()+
 stat_summary( fun.data = "mean_sdl",  fun.args = list(mult = 1), 
    geom = "pointrange", color = "black", size = 0.1)+
  xlab("Burrow") + ylab("Temperature (°C)")+
  plottheme+
  geom_hline(data = sp.TNZ, aes(yintercept = Temperature, colour = TNZ), linetype = "dashed", size = 1)+
  scale_colour_manual(values = c("#414487FF","#89B4C7","#E16462FF","#FCA636FF"))+
...
43.15 - (6.58*log10(490)) - (0.26*47.267) # = 13.15909
