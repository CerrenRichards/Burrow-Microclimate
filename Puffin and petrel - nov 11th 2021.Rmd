...
plot(m5.0_puffinAEB)
summary(m5.0_puffinAEB)

plot(atpu$Temperature, atpu$Wind.Speed)
plot(atpu$Wind.Speed, atpu$Temperature)


vis.gam(m5.0_puffinAEB, view = c("Temperature", "Wind.Speed"), plot.type = "contour")
...
# Explore the extreme temperature range to see whether it is comparable between the species
extreme %>%
  group_by(Species, Extreme) %>%
  summarise(mean = mean(Temperature), 
            sd = sd(Temperature))
...
ggplot(extreme, aes(x = Species, y = Temperature)) +
  geom_boxplot( aes(fill = buffer_kestrel>0))+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.title.x = element_text(vjust = -1))+
  theme(legend.position = "none")+
  facet_wrap(~Extreme, scales = "free_y")+
  theme(strip.text = element_text(size = 16, color = "white", face = "bold"), 
        strip.background = element_rect(fill="black"))
