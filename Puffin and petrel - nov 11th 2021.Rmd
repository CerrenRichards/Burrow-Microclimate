...
m17 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts", k = 4) + # Weather 
                  ...
                  s(scale(Enterance_Area),bs="ts") + # Burrow features
...
m16 <- gam(data = lssp,
           Inside~s(scale(Temperature),  k = 4) + # Weather 
                  ...
                  s(scale(Enterance_Area)) + # Burrow features
...
lme1 <- lme(data = lssp,
                   Inside ~ Temperature + Wind.Speed + Enterance_Area + volume + Canopy,
                   random = ~ 1|Burrow,
...
m1_puffin <- gam(data = atpu,
             Inside~s(scale(Temperature), bs="ts") + # Weather
                    s(scale(Wind.Speed), bs="ts") +  # Weather
                    s(scale(Enterance_Area), bs="ts") + # Burrow
                    s(scale(volume), bs="ts") + # Burrow features
...
m5.1_puffinAEB <- gam(data = atpu,
             Inside~scale(Temperature) + scale(Temperature) : scale(Wind.Speed) + # Environment
               s(scale(Wind.Speed)) +
                  scale(Enterance_Area) + # Burrow features
...
plot(m7.2_puffinAEB, scheme = 2)

gam.check(m7.2_puffinAEB)
plot(m7_puffinAEB)
summary(m6.1_puffinAEB)
...
vis.gam(m8.1_puffinAEB, view = c("Temperature", "Wind.Speed"),
    theta = 700, n.grid = 40, lwd = 0.4, too.far = 0.1)

plot(m5.0_puffinAEB, scheme = 2)
...
heat_ATPU <- extreme_heat_ATPU %>% group_by(Burrow) %>% 
  summarise(Buffer = mean(buffer_kestrel),
            ...
            Height = mean(Height),
            Depth = mean(Depth),
            Enterance_Area = mean(Enterance_Area),
            Canopy = mean(Canopy),
...
m0_heat_atpu <- lm(data = heat_ATPU, 
                   Buffer.med ~ volume + Enterance_Area)
...
m2_heat_atpu <- lm(data = heat_ATPU, 
                   Buffer.med ~ log(volume) + log(Enterance_Area))
...
m1_heat_lssp <- gls(data = heat_LSSP, 
                   Buffer.med ~ Canopy + volume + Enterance_Area)
...
m1_heat <- lme(data = heat,
              Buffer ~  volume + Enterance_Area + Canopy,
