...
heat_LSSP$Burrow <- as.character(heat_LSSP$Burrow)
heat_ATPU$Burrow <- as.character(heat_ATPU$Burrow)
cold_LSSP$Burrow <- as.character(cold_LSSP$Burrow)
cold_ATPU$Burrow <- as.character(cold_ATPU$Burrow)

heat <- bind_rows(heat_LSSP, heat_ATPU)
cold <- bind_rows(cold_LSSP, cold_ATPU)

# Drop rows of burrow with NA
library(tidyverse)
cold <- cold %>% drop_na()
heat <- heat %>% drop_na(Width)
...
# Visualise trends
ggplot(cold_ATPU, aes(x = log(volume), y = Buffer)) +
                  geom_point()+
...
m1_heat_atpu <- gls(data = heat_ATPU, 
                   Buffer ~ log(volume) + log(Enterance_Area))
...
hist(log(heat_ATPU$Enterance_Area))
...
m1_heat_lssp <- gls(data = heat_LSSP, 
                   Buffer ~ log(Canopy) + volume + log(Enterance_Area))

m1_heat_lssp <- gls(data = heat_LSSP, 
                   Buffer ~ Canopy + volume + Enterance_Area)
...
ggplot(heat_LSSP, aes(x = log(Canopy), y = Buffer)) +
  geom_point()+
  ylab(bquote(T[Buffer]~"(°C)"))+
