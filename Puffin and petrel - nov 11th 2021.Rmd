...
m2 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts", k = 4) + # Weather 
                  ...
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(Time, bs="cc") + # Time
                  s(julian, bs="ts", k = 6) + # Date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 

windmean <- lssp %>% group_by(julian) %>% summarise(wind = mean(Wind.Speed))
...
## Explore weather variable as linear terms 
## Linear terms increase the AIC score
## R2 = 0.67
## REML = 73938
m5 <- gam(data = lssp,
           Inside~Temperature + # Weather 
...
AIC(m4, m3, m6, m7) # m2 reduces the AIC
