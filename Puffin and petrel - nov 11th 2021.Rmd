...
# Explore the data
hist(model_df_short$buffer_logger)
hist(model_df_short$buffer_kestrel)
hist(model_df_short$Outside)
hist(model_df_short$Inside)
hist(model_df_short$Temperature)
hist(weather$Temperature)
hist(model_df_short$diff)
...
lssp_cor <- filter(model_df_short, Species == "Leach's storm-petrel")
atpu_cor <- filter(model_df_short, Species == "Atlantic puffin")

cor.test(lssp_cor$Temperature, lssp_cor$Inside) # R = 0.35 [0.34-0.36] p < 0.001
cor.test(atpu_cor$Temperature, atpu_cor$Inside) # R = 0.38 [0.37-0.39] p < 0.001
