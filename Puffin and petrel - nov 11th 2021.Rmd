...
1. Format the PUFFIN temperature data
    * Combine all the temperature .csv files
    * Set up the date and time correctly
    * Match the logger numbers to the data

2. Format the PETREL temperature data
    * Combine all the temperature .csv files
    * Set up the date and time correctly

3. Join the formatted puffin and petrel data
...
5. Calculate the burrow temperature buffering

6. Modelling
    * MODEL - Predict drivers of burrow buffering capacity
    * MODEL - Predict drivers of burrow microclimate
    * MODEL - Overperforming burrows
...
##______________________________________________________________
## 2. Format the PUFFIN temperature data
##______________________________________________________________
...
Some of the loggers were not found at the end of the season. They were likely kicked out by the puffins as they excavated the burrows, or physically removed. 
...
##______________________________________________________________
## 3. Format the PETREL temperature data
##______________________________________________________________
...
## MODEL - Predict drivers of burrow microclimate (Notes)
...
m14 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts") + s(scale(Wind.Speed),bs="ts") +  # Environment
                  s(scale(Entrance_Area),bs="ts") + s(scale(volume),bs="ts") + # Burrow features
...
library(DHARMa); library(lme4); library(mgcViz); library(mgcv)

# Visualise the VIF
devtools::install_github("samclifford/mgcv.helper")
library(mgcv.helper)

mgcv.helper::vif.gam(m14) ## Errors here
...
# Information on bs = ts:
# https://stat.ethz.ch/R-manual/R-devel/library/mgcv/html/smooth.construct.tp.smooth.spec.html?fbclid=IwAR190DaxZpNNYVPhjDnQCPJx9cjDY2Ar3OFPRQNQiGZMMX1STumBjVR_aBw

m17 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts", k = 4) + # Weather 
                  s(scale(Wind.Speed),bs="ts", k = 4) +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(scale(Time), bs="cc") + # Time
                  s(scale(julian), bs="ts", k = 6) + # Date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 

m16 <- gam(data = lssp,
           Inside~s(scale(Temperature),  k = 4) + # Weather 
                  s(scale(Wind.Speed), k = 4) +  # Weather
                  s(scale(Entrance_Area)) + # Burrow features
                  s(scale(volume)) + # Burrow features
                  s(scale(Canopy)) + # Habitat
                  s(scale(Time), bs="cc") + # Time
                  s(scale(julian), k = 6) + # Date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 

## The GAMM model to compare

#Comparing the `gam` model with burrow as a random effect spline against a `gamm` model  produced #effectively equivalent model outputs. However, we selected the `gam` model because it allows for #tests of random effect terms against a null of 0 variance and was also a better fit than the `gamm` #model. 


# Produced effectively equivalent outputs but the fit was worse
m2 <- gamm(data = lssp,
           Inside~s(scale(Temperature), bs="ts", k = 4) + # Weather 
                  s(scale(Wind.Speed),bs="ts", k = 4) +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  ...
                  s(scale(Time), bs="cc") + # Time
                  s(scale(julian), bs="ts", k = 6), # Date
                  random=list(Burrow=~1), # random effect
                  method = "REML") 
...
plot(m16)
plotResiduals(m17)
summary(m16) # explains 66.9% of deviance

# Exploring how much data and time explain
m18 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts", k = 4) + # Weather
                  s(scale(Wind.Speed),bs="ts", k = 4) +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
...
summary(m18) # 47.3% - date and time explains 19.6% of the deviance
...
summary(m19) # 66.9% - canopy explains nothing
...
# Temp
m23 <- gam(data = lssp,
           Inside~s(scale(Wind.Speed),bs="ts", k = 4) +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(scale(Time), bs="cc") + # Time
...
# Making them linear terms
m19 <- gam(data = lssp,
           Inside~Temperature + # Weather 
                  poly(Wind.Speed) +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(Burrow, bs="re"), # random effect
...
summary(m20) # 45.7% - canopy explain 

anova(m18,m19)
# polynomial (wind) or linear (temp)
# lme 
# pq = 5 for corarma
# take out terms and see how things are explained


# making temp and wind linear terms didnt improve the model


lme1 <- lme(data = lssp,
                   Inside ~ Temperature + Wind.Speed + Entrance_Area + volume + Canopy,
                   random = ~ 1|Burrow,
...
ggplot(lssp, aes(x = scale(Canopy), y = scale(Inside))) +
  geom_point()+
    plottheme
...
# Define the burrow ID as a character 
atpu$Burrow <- as.factor(atpu$Burrow)

m1_puffin <- gam(data = atpu,
             Inside~s(scale(Temperature), bs="ts") + 
                    s(scale(Wind.Speed),bs="ts") +  # Environment
                    s(scale(Entrance_Area),bs="ts") + 
                    s(scale(volume),bs="ts") + # Burrow features
                    s(scale(Time), bs="cc") + 
                    s(scale(julian), bs="ts")+ # Time and date
                    s(Burrow, bs="re"), # random effect
                    method = "REML") 
...
# edf is close to 9 for temperature, entrance area, volume and julian date
# so we will double the knots to 20
m2_puffin <- gam(data = atpu,
             Inside~s(scale(Temperature), bs="ts", k = 4) + # Environment
                  s(scale(Wind.Speed),bs="ts", k = 4) +  # Environment
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Time), bs="cc") + 
                  s(scale(julian), bs="ts")+ # Time and date
...
testDispersion(m1_puffin)
testZeroInflation(m1_puffin)
hist(resid(m2_puffin))
plot(m1_puffin)
summary(m1_puffin)


# Visualise trends
ggplot(atpu, aes(x = Wind.Speed, y = Inside)) +
                  geom_point()+
...
# Load the packages
library(DHARMa); library(lme4); library(mgcViz); library(mgcv); library(dsm)
...
# DHARMa package residual exploration
testResiduals(m1)
...
# Wind speed and temperature appear to show a linear response
# We will therefore run the following model with them as linear terms
plot(lssp$Wind.Speed, lssp$Inside)
plot(lssp$Temperature, lssp$Inside)

## Explore weather variables as linear terms 
## Linear terms decrease the model fit:
## R-sq = 0.67
## REML = 73938
## AIC = 147572.9
m5 <- gam(data = lssp,
...
# Changing the knots of the initial model to explore whether this improves the fit
# Improvement from the linear terms, but not from the initial model:
## R-sq = 0.673
## REML = 73676
## AIC = 147046.9
m3 <- gam(data = lssp,
...
# Explore changes in knots for only the wind speed term
# because there is a slight inflection upwards in the previous models
# for wind speed due to lack of data at higher wind speeds
m6 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts") + # Weather 
                  s(scale(Wind.Speed),bs="ts", k = 5) +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(Time, bs="cc") + # Time
...
# Decrease knots to reduce the inflection at higher wind speeds
m7 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts") + # Weather 
                  s(scale(Wind.Speed),bs="ts", k = 4) +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
...
m10 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts") + # Weather 
                  ...
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(Time, bs="cc", k = 12) + # Time
                  s(julian, bs="ts", k = 15) + # Date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 
...
m11 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts") + # Weather 
                  ...
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(Time, bs="cc", k = 10) + # Time
                  s(julian, bs="ts", k = 12) + # Date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 
...
m12 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts", k = 20) + # Weather 
                  s(scale(Wind.Speed),bs="ts") +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
...
m13 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts", k = 40) + # Weather 
                  s(scale(Wind.Speed),bs="ts") +  # Weather
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
...
# Aic
# Does it change the others
# Dharma - test.autocorrelation ??

AIC(m4, m12, m13, m14, m15, m16) 
...
m1_petrel <- gam(data = lssp,
             Inside~s(scale(Temperature), scale(Wind.Speed)) +
                  s(scale(Entrance_Area), bs="ts") + # Burrow
                  s(scale(volume), bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(Time, bs="cc") + 
                  s(julian, bs="ts")+ # Time and date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 



m1_petrelb <- gam(data = lssp,
             Inside~s(Temperature, Wind.Speed) +
                  scale(Entrance_Area) + # Burrow
                  scale(volume) + # Burrow features
                  scale(Canopy) + # Habitat
                  s(Time, bs="cc") + 
                  s(julian, bs="ts")+ # Time and date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 

vis.gam(m1_petrelb, view = c("Temperature", "Wind.Speed"),
    theta = 300, n.grid = 40, lwd = 0.4, too.far = 0.1)

AIC(m4, m1_petrel)
summary(m1_petrel)
plot(m1_petrel, scheme = 2, pages = 1)

## THIS MODEL FOR LINEAR
m3_petrel <- gam(data = lssp,
             Inside~scale(Temperature) * scale(Wind.Speed) + # Environment
                  s(scale(Entrance_Area), bs="ts") + # Burrow
                  s(scale(volume), bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(Time, bs="cc") + 
...
plot(atpu$Temperature, atpu$Inside)
...
m1.3_puffin <- gam(data = atpu.wind,
             Inside~s(scale(Temperature), bs="ts") + # Weather
                    s(scale(Wind.Speed), bs="ts") +  # Weather
                    s(scale(Entrance_Area), bs="ts") + # Burrow
                    s(scale(volume), bs="ts") + # Burrow features
...
m1.4_puffin <- gam(data = atpu.wind,
             Inside~s(scale(Temperature), bs="ts") + # Weather
                    s(scale(Wind.Speed), bs="ts") +  # Weather
                    s(scale(Entrance_Area), bs="ts") + # Burrow
                    s(scale(volume), bs="ts") + # Burrow features
                    s(Time, bs="cc") + # Time and date
                    s(julian, bs="ts", k = 40)+ # Time and date
                    s(Burrow, bs="re"), # random effect
                    method = "REML") 



m5_puffin <- gam(data = atpu,
             Inside~s(scale(Temperature), bs="ts") + # Environment
                  s(scale(Wind.Speed),bs="ts") +  # Environment
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
                  s(Time, bs="cc") + 
...
m5.1_puffinAEB <- gam(data = atpu,
             Inside~scale(Temperature) + scale(Temperature) : scale(Wind.Speed) + # Environment
               s(scale(Wind.Speed)) +
                  scale(Entrance_Area) + # Burrow features
                  scale(volume) + # Burrow features
                  s(Time, bs="cc") + 
                  s(julian, bs="ts", k = 5)+ # Time and date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 



# Modeling the effects left over
m5.1_puffinAEB <- gam(data = atpu.wind,
             Inside~scale(Temperature) + scale(Temperature) : scale(Wind.Speed) + # Environment
               s(scale(Wind.Speed)) +
                  scale(Entrance_Area) + # Burrow features
                  scale(volume) + # Burrow features
...
m5.2_puffinAEB <- gam(data = atpu.wind,
             Inside ~ scale(Temperature) * scale(Wind.Speed) + # Environment
                  scale(Entrance_Area) + # Burrow features
                  scale(volume) + # Burrow features
                  s(Time, bs="cc") + 
...
# gam to justify fitting a linear term 
# constructed with particular criteria 
AIC(m1_puffin,m5.0_puffinAEB, m6.0_puffinAEB, m6.1_puffinAEB)

m6.1_puffinAEB <- gam(data = atpu.wind,
             Inside~s(scale(Temperature), scale(Wind.Speed)) +
                  s(scale(Entrance_Area), bs="ts") + # Burrow
                  s(scale(volume), bs="ts") + # Burrow features
                  s(Time, bs="cc") + 
...
m5.0_puffinAEB <- gam(data = atpu.wind,
             Inside~scale(Temperature) * scale(Wind.Speed) + # Environment
                  scale(Entrance_Area) + # Burrow features
                  scale(volume) + # Burrow features
                  s(Time, bs="cc") + 
...
m9.0_puffinAEB <- gam(data = atpu.wind,
             Inside~Temperature * Wind.Speed + # Environment
                  scale(Entrance_Area) + # Burrow features
                  scale(volume) + # Burrow features
                  s(Time, bs="cc") + 
                  s(julian, bs="ts")+ # Time and date
...
## THIS MODEL FOR LINEAR
m6.0_puffinAEB <- gam(data = atpu.wind,
             Inside~scale(Temperature) * scale(Wind.Speed) + # Environment
                  s(scale(Entrance_Area), bs="ts") + # Burrow
                  s(scale(volume), bs="ts") + # Burrow features
                  s(Time, bs="cc") + 
...
m7_puffinAEB <- gam(data = atpu.wind,
             Inside~s(Temperature, Wind.Speed) +
                  s(scale(Entrance_Area), bs="ts") + # Burrow
                  s(scale(volume), bs="ts") + # Burrow features
                  s(Time, bs="cc") + 
...
m7.2_puffinAEB <- gam(data = atpu.wind,
             Inside~s(Temperature, Wind.Speed, k = 60) +
                  s(scale(Entrance_Area), bs="ts") + # Burrow
                  s(scale(volume), bs="ts") + # Burrow features
                  s(Time, bs="cc") + 
                  s(julian, bs="ts")+ # Time and date
...
## THIS MODEL FOR SPLINE
m7.3_puffinAEB <- gam(data = atpu.wind,
...
m8.0_puffinAEB <- gam(data = atpu.wind,
             Inside~s(Temperature, Wind.Speed)  + # Environment
                  scale(Entrance_Area) + # Burrow features
                  scale(volume) + # Burrow features
                  s(Time, bs="cc") + 
                  s(julian, bs="ts")+ # Time and date
...
m8.1_puffinAEB <- gam(data = atpu.wind,
             Inside~s(Temperature, Wind.Speed, k = 60)  + # Environment
                  scale(Entrance_Area) + # Burrow features
                  scale(volume) + # Burrow features
                  s(Time, bs="cc") + 
                  s(julian, bs="ts")+ # Time and date
...
m8_puffin <- gam(data = atpu.wind,
           Inside~s(Temperature2, Wind.Speed2, k = 60) +
                  scale(Entrance_Area) + # Burrow features
                  scale(volume)+ # Burrow features
                  s(Time, bs="cc") + # Time
                  s(julian, bs="ts", k = 20) + # Date
...
atpu.wind$Temperature. <- scale(atpu.wind$Temperature)
atpu.wind$Wind.Speed.  <- scale(atpu.wind$Wind.Speed)
atpu.wind$Entrance_Area2 <- scale(atpu.wind$Entrance_Area)
atpu.wind$volume2 <- scale(atpu.wind$volume)

m8_puffin2 <- gam(data = atpu.wind,
           Inside~s(Temperature. , Wind.Speed. , k = 60) +
                  s(Entrance_Area2, bs="ts") + # Burrow features
                  s(volume2, bs="ts")+ # Burrow features
                  s(Time, bs="cc") + # Time
                  s(julian, bs="ts", k = 20) + # Date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 
...
par(mfrow = c(1,2))
vis.gam(m8_puffin2, view = c("Temperature.", "Wind.Speed."),
    theta = 700, n.grid = 40, lwd = 0.4, too.far = 0.1)

vis.gam(m8_puffin2, view = c("Temperature.", "Wind.Speed."),
    theta = 550, n.grid = 40, lwd = 0.4, too.far = 0.1)

m8.2_puffinAEB <- gam(data = atpu.wind,
             Inside~s(Temperature, Wind.Speed, k = 60),# Environment
                  method = "REML") 


plot(m7.3_puffinAEB, scheme = 2, pages = 1)

plot(m7.3_puffinAEB, scheme = 2)

gam.check(m7.3_puffinAEB)
plot(m7_puffinAEB)
summary(m7.3_puffinAEB)

plot(atpu$Temperature, atpu$Wind.Speed)
plot(atpu.wind$Wind.Speed, atpu.wind$Temperature)


vis.gam(factor_interact, theta = 120, n.grid = 50, lwd = 0.4)

vis.gam(m5.0_puffinAEB, view = c("Temperature", "Wind.Speed"), 
        plot.type = "contour")

vis.gam(m5.0_puffinAEB, view = c("Temperature", "Wind.Speed"),
    theta = 100, n.grid = 40, lwd = 0.4)


par(mfrow = c(1,2))
vis.gam(m8.1_puffinAEB, view = c("Temperature2", "Wind.Speed2"),
    theta = 700, n.grid = 40, lwd = 0.4, too.far = 0.1)
...
plot(m8_puffin2, scheme = 2)

predict.gam()


res <- simulateResiduals(m7.3_puffinAEB)

testResiduals(res, plot = T)


# predict

m6_puffin <- gam(data = atpu,
             Inside~s(scale(Temperature), bs="ts") + # Environment
                  s(scale(Wind.Speed),bs="ts") +  # Environment
                  s(scale(Entrance_Area),bs="ts") + # Burrow features
                  s(scale(volume),bs="ts") + # Burrow features
...
# model checks
AIC(m1_puffin, m2_puffin, m3_puffin, m4_puffin) 
AIC(m1.2_puffin, m1.3_puffin) 
plot(m1.3_puffin)
summary(m5_puffin)
...
# constructed with particular criteria 
# look at predict.gam
## specify as time or use unique nums
# re for daytime, or just day
res = simulateResiduals(m5_puffin)
res = recalculateResiduals(res, group = atpu$datetime2)

testTemporalAutocorrelation(res, time = unique(atpu$datetime2))


res = simulateResiduals(m8_puffin2)
...
## MODEL - Predict drivers of burrow buffering capacity (not used)

```{r}
## PETREL

# Build initial model
m1_buff <- gam(data = lssp,
           buffer_kestrel~s(scale(Wind.Speed),bs="ts") +  # Environment
                  s(scale(Entrance_Area),bs="ts") + s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + s(scale(Ground_cover), bs="ts") + # Habitat
                  s(scale(Time), bs="cc") + s(scale(julian), bs="ts")+ # Time and date
                  s(Burrow, bs="re"), # random effect
...
mean(extreme_cold_LSSP$buffer_logger) # 6.83 deg C
sd(extreme_cold_LSSP$buffer_logger) #1.34
...
## Mean and SD for external burrow logger buffering
mean(extreme_heat_LSSP$buffer_logger) # -8.274 C
sd(extreme_heat_LSSP$buffer_logger) #3.461
...
Here we build two models for each species: one for the extreme cold and one for the extreme warm events. 

We first calculate the mean buffering temperature per burrow during extreme events (90th percentile). Following this, we build GLS models to identify unique features of burrows that are over and under performing during extreme events. 


Mean buffering temperature was included as the dependent variable. Burrow characteristics (entrance area, volume) and canopy cover (Leach's storm-petrels only) were included as predictors.


```{r}

## Summarise the mean buffering temperatures per burrow

## PUFFIN - HEAT EXTREME
heat_ATPU <- extreme_heat_ATPU %>% group_by(Burrow) %>% 
  summarise(Buffer = mean(buffer_kestrel),
            Buffer.med = median(buffer_kestrel),
            Elevation = mean(Elevation),
            Burrow.orientation = mean(Burrow.orientation),
            Width = mean(Width),
            Height = mean(Height),
            Depth = mean(Depth),
            Entrance_Area = mean(Entrance_Area),
            Canopy = mean(Canopy),
            Ground_cover = mean(Ground_cover))

# Calculate volume
heat_ATPU$volume <- pi * (heat_ATPU$Width/2) * (heat_ATPU$Height/2) * heat_ATPU$Depth
heat_ATPU$Species <- "ATPU"

## PUFFIN - COLD EXTREME
cold_ATPU <- extreme_cold_ATPU %>% group_by(Burrow) %>% 
  summarise(Buffer = mean(buffer_kestrel),
            Buffer.med = median(buffer_kestrel),
            Elevation = mean(Elevation),
            Burrow.orientation = mean(Burrow.orientation),
...
# Calculate volume                                                         
cold_ATPU$volume <- pi * (cold_ATPU$Width/2) * (cold_ATPU$Height/2) * cold_ATPU$Depth
cold_ATPU$Species <- "ATPU"

## PETREL - COLD EXTREME
cold_LSSP <- extreme_cold_LSSP %>% group_by(Burrow) %>% 
  summarise(Buffer = mean(buffer_kestrel),
            Buffer.med = median(buffer_kestrel),
            Elevation = mean(Elevation),
            Burrow.orientation = mean(Burrow.orientation),
...
# Calculate volume                                                           
cold_LSSP$volume <- pi * (cold_LSSP$Width/2) * (cold_LSSP$Height/2) * cold_LSSP$Depth
cold_LSSP$Species <- "LSSP"

## PETREL - HEAT EXTREME
heat_LSSP <- extreme_heat_LSSP %>% group_by(Burrow) %>% 
  summarise(Buffer = mean(buffer_kestrel),
            Buffer.med = median(buffer_kestrel),
            Elevation = mean(Elevation),
            Burrow.orientation = mean(Burrow.orientation),
...
# Calculate volume                                                               
heat_LSSP$volume <- pi * (heat_LSSP$Width/2) * (heat_LSSP$Height/2) * heat_LSSP$Depth
heat_LSSP$Species <- "LSSP"

```


## PUFFIN - HEAT EXTREME MODEL
```{r}

# Burrow 36 has NA values for burrow features
# so we will remove this burrow from the analysis
heat_ATPU <- heat_ATPU %>% drop_na()


# Build initial model
m0_heat_atpu <- lm(data = heat_ATPU, 
                   Buffer.med ~ volume + Entrance_Area)
# Check model fit - Not a good fit
stats::qqnorm(m0_heat_atpu)
plot(m0_heat_atpu)
hist(resid(m0_heat_atpu))
summary(m0_heat_atpu)

# Build initial model
m1_heat_atpu <- gls(data = heat_ATPU, 
                   Buffer.med ~ volume + Entrance_Area)

# Check model fit - Not a good fit
stats::qqnorm(m1_heat_atpu)
plot(m1_heat_atpu)
hist(resid(m1_heat_atpu))
summary(m1_heat_atpu)

# Transform the predictors
m2_heat_atpu <- lm(data = heat_ATPU, 
                   Buffer.med ~ log(volume) + log(Entrance_Area))

# Check model fit 
# The transformations improved the fit slightly
stats::qqnorm(m2_heat_atpu)
plot(m2_heat_atpu)
hist(resid(m2_heat_atpu))

# Model summary
summary(m2_heat_atpu)

# Visualise trends
ggplot(cold_ATPU, aes(x = log(volume), y = Buffer.med)) +
                  geom_point()+
                  ylab(bquote(T[Buffer]~"(°C)"))+
                  plottheme+
                  theme(legend.position = "none")
...
## PUFFIN - COLD EXTREME MODEL
```{r}

# Burrow 36 has NA values for burrow features
# so we will remove this burrow from the analysis
cold_ATPU <- cold_ATPU %>% drop_na()

# Build model
m1_cold_atpu <- gls(data = cold_ATPU, 
                   Buffer.med ~ volume + Entrance_Area)

# Check model fit
stats::qqnorm(m1_cold_atpu)
plot(m1_cold_atpu)
hist(resid(m1_cold_atpu))

# Model summary
summary(m1_cold_atpu)

# Visualise trends
ggplot(cold_ATPU, aes(x = volume, y = Buffer.med)) +
                  geom_point()+
                  ylab(bquote(T[Buffer]~"(°C)"))+
                  plottheme+
                  theme(legend.position = "none")
...
## PETREL - HEAT EXTREME MODEL
```{r}

# Build model
m1_heat_lssp <- gls(data = heat_LSSP, 
                   Buffer.med ~ Canopy + volume + Entrance_Area)


# Check model fit
stats::qqnorm(m1_heat_lssp)
hist(resid(m1_heat_lssp))
plot(m1_heat_lssp)


# Model Summary
summary(m1_heat_lssp)

# Visualise trends
ggplot(heat_LSSP, aes(x = Canopy, y = Buffer.med)) +
                  geom_point()+
                  ylab(bquote(T[Buffer]~"(°C)"))+
                  plottheme+
                  theme(legend.position = "none")
...
## PETREL - COLD EXTREME MODEL
```{r}

# Build model
m1_cold_lssp <- gls(data = cold_LSSP, 
                   Buffer.med ~  Canopy + volume + Entrance_Area)


# Check model fit
stats::qqnorm(m1_cold_lssp)
hist(resid(m1_cold_lssp))
plot(m1_cold_lssp)


# Model summary
summary(m1_cold_lssp)


```

## Modelling help from AB and DF
```{r}
...
m1 <- gamm(data = lssp,
           buffer_kestrel~s(Entrance_Area) + s(Height) + s(Width) + s(Depth),
           random = list(Burrow = ~1))

m1 <- gamm(data = lssp,
           buffer_kestrel~s(Entrance_Area) + s(volume),
           random = list(Burrow = ~1))
...
m3 <- gamm(data = atpu,
           Inside~s(Temperature)+ s(Entrance_Area) + s(volume) +s(Time),
           random = list(Burrow = ~1))
...
plot(model_df_short$Entrance_Area, model_df_short$buffer_kestrel)
...
m1 <- gamm(data = lssp,
           Inside~s(scale(Temperature)) + s(scale(Wind.Speed)) +  # Environment
                  s(scale(Entrance_Area)) + s(scale(volume)) + # Burrow features
...
m13 <- gamm(data = lssp,
           residuals_lssp~s(scale(Temperature)) + s(scale(Wind.Speed))+ # Environment
                   s(scale(Entrance_Area)) + s(scale(Height)) + # Burrow features
                  s(scale(Canopy)) + s(scale(Ground_cover)), # Habitat 
           random = list(Burrow = ~1))
...
plot(lssp$Entrance_Area, lssp$residuals_lssp)
...
m8 <- gam(data = mean,
           mean_inside~ # Environment
                  s(scale(Entrance_Area), k=4) + s(scale(volume), k=4) +
...
m3 <- gamm(data = lssp,
           buffer_kestrel~ s(Wind.Speed) +  # Environment
                  s(Entrance_Area) + s(volume) + # Burrow features
...
m7 <- gamm(data = lssp,
           Inside~s(Temperature) + s(Wind.Speed) +  # Environment
                  s(Entrance_Area) + s(Height) + s(Width) + s(Depth)+ # Burrow features
                  s(Canopy) + s(Ground_cover) + # Habitat
                  s(Time) + s(julian), # Time and date
           random = list(Burrow = ~1))
...
## Heat model two species

m1_heat <- lme(data = heat,
              Buffer ~  volume + Entrance_Area + Canopy,
...
ggplot(heat, aes(x = Canopy, y = Buffer, group = Species)) +
...
ggplot(weather2, aes(x = Date, y = Time, fill = Temperature)) +
...
# Plot without the air temperature
tempall <- ggplot(data = season, aes(x = datetime, y = mean))+
...
# Filter burrows that can't be used for the analysis
ATPU_LSSP <- ATPU_LSSP %>% filter(!Burrow %in% c(1, 2, 3, 9, 15, 21, 27, 29))
