...
temps <- temps %>% filter(datetime >= "2021-07-16 15:00:00") %>% 
...
# For modelling analyses, we only need the internal burrow temperatures
# We will filter the ATPU_LSSP dataframe to only include the internal logger data

ATPU_LSSP %>% select(datetime, Date, Time, Species, Burrow, Location, Plot, temp) %>% 
...
weather$datetime2 <- weather$DateTime

# Match the weather data and the burrow logger data
weather_burrow <- left_join(weather, ATPU_LSSP, by = "datetime2")
...
open <- ATPU_LSSP %>% filter(Plot %in% c("Slope", "Open"))
forest <- ATPU_LSSP %>% filter(Plot %in% c("Forest", "Near Forest"))

# add extra column so they can join
weather1$datetime2 <- weather1$DateTime
weather2$datetime2 <- weather2$DateTime

# Join the kestrel weather data and the temp logger data
forest_join <- left_join(forest, weather1, by = "datetime2")
open_join <- left_join(open, weather2, by = "datetime2")


# Summarise the mean temperature and standard deviation for inside and outside the burrows
season <- forest_join %>%
  group_by(Species, Location, datetime2) %>%
...
heat <- ggplot(weather1, aes(x = Date, y = Time, fill = Temperature)) +
  theme_bw(base_size = 15)+
