...
burrows <- burrows %>% select(Species, Burrow, Elevation:Enterance_Area)
...
ggplot(data = maxmin_season, aes(x = Time, y = Mean, group = Location))+
  geom_ribbon(aes(ymin=Mean-sd, ymax=Mean+sd, fill = Location), alpha = 0.35)+
  geom_line(aes(colour = Location, linetype = Location),  size = 1.5)+
  xlab("Time of Day (h)") + ylab("Temperature (°C)")+
  scale_x_continuous(breaks=seq(0,23,4))+
  plottheme+
  theme(legend.title = element_blank())+ 
...
ggplot()+
 # geom_line(data = maxmin_season, aes(x = Time, y = Min, 
 #                                     group = Location, linetype = Location, colour = Location))+
...
"#43A407",  "dodgerblue", "#4E342E")

 scale_fill_manual(values = c("#E0DEBF",  "#4C837B","#04263B" ))+
...
# Create a new dataframe for the changepoint data to be placed in

x <- filter(ATPU_LSSP, Location == "Inside" & Burrow == "17")
x <- filter(ATPU_LSSP, Location == "Inside" & Burrow == "128")

x1 <- cpt.meanvar(x$temp, method = "BinSeg", Q=3)
...
# Run the changepoint analysis
x1 <- cpt.meanvar(x$temp)

# Identify the change points
cpt[i] <- cpts(x1)
burrow[i] <- i
Date[i] <- x$Date[cpts(x1)]
}

# Create the dataframe containing all the infomation
petrelcpt <- data.frame(burrow, cpt, Date) %>% arrange(cpt)




cpts(cpt.meanvar(x$temp), method = "BinSeg", Q=3)[3]
...
# Identify the change points
petrelcpt$cpt[i] <- cpts(x1)
petrelcpt$burrow[i] <- i
petrelcpt$Date[i] <- x$Date[cpts(x1)]
#petrelcpt <- data.frame(burrow, cpt)
}
...
for(i in 1:unique(puffin$Burrow)){
...
test <- rbind(petrelcpt, petrelcpt)
...
#for(i in 1:nrow(puffin)){



# Identify the date and time of the changepoint
Date <- x$Date[cpts(x1)]
datetime <- x$datetime[cpts(x1)]

# Add a burrow column
burrow <- i

# Create a dataframe with all the information
petrelcpt <- data.frame(burrow, cpt, Date, datetime)
