...
ggplot(data = maxmin_season, aes(x = Time, y = Mean, group = Location, linetype = Location))+
  #geom_line(aes(colour = Location, linetype = Location))+
  geom_ribbon(aes(ymin=Min, ymax=Max, fill = Location, linetype = Location), alpha = 0.35)
  xlab("Time of Day (h)") + ylab("Temperature (°C)")+
  scale_x_continuous(breaks=seq(0,23,4))+
  plottheme+
  theme(legend.title = element_blank())+ 
  #scale_colour_manual(values = c("#43A407", "#4E342E", "red"))+
  #scale_fill_manual(values = c("#43A407", "#4E342E", "red"))+
  theme(legend.position = c(0.9, 0.8))+
  facet_wrap(~Species, scales = "free_x")+
  theme(strip.text.x = element_text(size = 16, color = "white", face = "bold"), 
        strip.background = element_rect(fill="black"))
