...
m17 <- gam(data = lssp,
           Inside~s(scale(Temperature), bs="ts", k = 4) + s(scale(Wind.Speed),bs="ts", k = 4) +  # Weather
                  s(scale(Enterance_Area),bs="ts") + s(scale(volume),bs="ts") + # Burrow features
                  s(scale(Canopy), bs="ts") + # Habitat
                  s(scale(Time), bs="cc") + s(scale(julian), bs="ts", k = 6)+ # Time and date
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 

plot(m17)
plotResiduals(m17)
summary(m17) # explains 66.9% of deviance
...
heat_ATOU$Burrow <- as.character(heat_ATPU$Burrow)
...
# Create the plot
ggplot()+
  geom_violin(data = maxmin2, aes(x = reorder(Burrow, -mean), 
                  y = Temperature, 
                  fill = Unit, colour = Unit))+
  scale_fill_manual(values = c("#E16E08", "#F9E07F","#89B4C7")) +
  scale_colour_manual(values = c( "#E16E08","#F9E07F","#89B4C7")) +
...
meanatpu <- mean %>% filter(Species == "Atlantic puffin") %>%
                            arrange(desc(mean))

meanlssp <- mean %>% filter(Species == "Leach's storm-petrel") %>%
                            arrange(desc(mean))

atpulevels <- maxmin2 %>% filter(Species == "Atlantic puffin") 
lssplevels <- maxmin2 %>% filter(Species == "Leach's storm-petrel")

atpulevels$Burrow <- factor(atpulevels$Burrow, levels=meanatpu$Burrow)
lssplevels$Burrow <- factor(lssplevels$Burrow, levels=meanlssp$Burrow)

maxmin2 <- rbind(atpulevels, lssplevels)

ggplot()+
  geom_violin(data = maxmin2, aes(x = Burrow, 
                  y = Temperature, 
                  fill = Unit, colour = Unit))+
  scale_fill_manual(values = c("#E16E08", "#F9E07F","#89B4C7")) +
  scale_colour_manual(values = c( "#E16E08","#F9E07F","#89B4C7")) +
