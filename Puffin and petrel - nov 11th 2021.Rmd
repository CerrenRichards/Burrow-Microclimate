...
petrelcpt <- data.frame(burrow, cpt,cpt3, Date) %>% arrange(cpt)
...
burrow[i] <- i
Date1[i] <- x$Date[cpt1[i]]
Date2[i] <- x$Date[cpt2[i]]
}

# Create the dataframe containing all the infomation
petrelcpt <- data.frame(burrow, cpt1,cpt2, Date1, Date2) %>% arrange(cpt1)
