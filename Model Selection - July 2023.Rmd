...
1.	Modelled GAM with burrow as a random factor, and date and time as predictors to account for autocorrelation 
a.	Model fit is good, except for autocorrelation
b.	Autocorrelation cannot be modelled with AR for GAM models
...
The model `m5_petrel` is highly autocorrelated, therefore, we will try and model with `bam` instead.
```{r}
