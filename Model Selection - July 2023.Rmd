---
title: "Supplementary Material 1 - Model Selection Procedure for Drivers of Burrow Microclimate"
...
## GAM
gam_ext_pet<- gam(data = extreme_heat_LSSP,
           buffer_kestrel~s(scale(Canopy), bs="ts") + # Weather 
                  s(scale(Enterance_Area),bs="ts") +  # Weather
                  s(scale(volume),bs="ts") +  # Weather
                  s(Burrow, bs="re"), # random effect
...
plot(gam_ext_pet)
summary(gam_ext_pet)
gam.check(gam_ext_pet)
acf(residuals(gam_ext_pet))


## GAMM
gamm_extreme_pet <- gamm(data = extreme_heat_LSSP,
                   buffer_kestrel~ s(scale(Canopy), bs="ts") +
                   s(scale(Enterance_Area),bs="ts") +  # 
                   s(scale(volume),bs="ts") +  # Weather
                   s(Burrow, bs="re"), # random effect
...
gam.check(gamm_extreme_pet$gam)
summary(gamm_extreme_pet$gam)
acf(residuals(gamm_extreme_pet$lme, type = "normalized"))





gam_ext_pet_col<- gam(data = extreme_cold_LSSP,
           buffer_kestrel~s(scale(Canopy), bs="ts") + # Weather 
                  s(scale(Enterance_Area),bs="ts") +  # Weather
                  s(scale(volume),bs="ts") +  # Weather
                  s(Burrow, bs="re"), # random effect
...
plot(gam_ext_pet_col)
summary(gam_ext_pet_col)
gam.check(gam_ext_pet_col)
acf(residuals(gam_ext_pet_col))



gam_ext_puffin<- gam(data = extreme_heat_ATPU,
           buffer_kestrel~s(scale(Enterance_Area),bs="ts") +  # Weather
                  s(scale(volume),bs="ts") +  # Weather
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 

summary(gam_ext_puffin)
plot(gam_ext_puffin)
gam.check(gam_ext_puffin)
acf(residuals(gam_ext_puffin))



gam_ext_puffin_col<- gam(data = extreme_cold_ATPU,
           buffer_kestrel~s(scale(Enterance_Area),bs="ts") +  # Weather
                  s(scale(volume),bs="ts") +  # Weather
                  s(Burrow, bs="re"), # random effect
                  method = "REML") 

summary(gam_ext_puffin_col)
plot(gam_ext_puffin_col)
gam.check(gam_ext_puffin_col)
acf(residuals(gam_ext_puffin_col))
...
form <- buffer_kestrel~ s(scale(Enterance_Area),bs="ts") +  # Weather
          s(scale(volume),bs="ts") +  # Weather
          s(Burrow, bs="re") # random effect

# Now deal with autocorrelation.
# First time through with no AR(1) process to get initial rho estimate
bam0_buffer <- bam(formula = form, data = extreme_cold_ATPU, method = "REML")
summary(bam0_buffer)
acf.vals <- acf(residuals(bam0_buffer))
...
gamm1_extreme <- gamm(data = extreme_cold_ATPU,
                   buffer_kestrel~ s(scale(Enterance_Area),bs="ts") +  # 
                   s(scale(volume),bs="ts") +  # Weather
                   s(Burrow, bs="re"), # random effect
                   corr = corAR1(form = ~ 1|Burrow),
...
gam.check(gamm1_extreme$gam)
summary(gamm1_extreme$gam)
acf(residuals(gamm1_extreme$lme, type = "normalized"))



gamm2_extreme <- gamm(data = extreme_heat_ATPU,
                   buffer_kestrel~ s(scale(Enterance_Area),bs="ts") +  # 
                   s(scale(volume),bs="ts") +  # Weather
                   s(Burrow, bs="re"), # random effect
                   corr = corAR1(form = ~ 1|Burrow),
...
gam.check(gamm2_extreme$gam)
summary(gamm2_extreme$gam)
acf(residuals(gamm2_extreme$lme, type = "normalized"))
