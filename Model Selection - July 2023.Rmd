...
1. GAM, BAM, and GAMM
2. Smooth terms, linear terms and interaction terms
3. Autocorrelation: AR(1) and date time spline fits 
4. Burrow ID as a random factor


In the final model, internal burrow temperature (Internal) was included as the dependent variable. The interaction between air temperature and wind speed was included as a smooth term. 

The final model was selected based on lowest AIC score and best fit. 

____________

A spline fits through time and Julian date were included to account for autocorrelation. All predictors were scaled and fitted with thin plate regression splines, except for time, which was fitted with a cyclic cubic regression spline to account for circular data. Burrow ID was added as a random factor, and defined with a random effects spline. 
...
bam0 <- bam(formula = form, data = lssp2, method = "REML")
summary(bam0)
...
summary(bam3)
acf(residuals(bam3)) # Correlated residuals
...
# Check model fit
plot(mgamm$gam,scheme = 2)
hist(resid(mgamm$gam))
gam.check(mgamm$gam)
plot(mgamm$lme)
```
