...
form <- Inside ~ s(Temperature, Wind.Speed, k = 60) +
  s(Time, bs = "cc") +
  s(julian, bs = "ts", k = 20)
# add the random effect for use in bam(). It is specified differently in gamm()
form <- update(base_form, . ~ . + s(Burrow, bs = "re") )

form <- buffer_kestrel~s(scale(Canopy), bs="ts") + # Weather 
          s(scale(Enterance_Area),bs="ts") +  # Weather
          s(scale(volume),bs="ts") +  # Weather
          s(Time, bs="cc") + # Time
          s(julian, bs="ts", k = 40) + # Date
...
# Now deal with autocorrelation.
# First time through with no AR(1) process to get initial rho estimate
bam0_buffer <- bam(formula = form, data = lssp2, method = "REML")
summary(bam0_buffer)
